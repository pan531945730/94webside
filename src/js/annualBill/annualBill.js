...
(function(window) {
    ...
    var getFlipBook = {
        ...
        sucFun: function(v) {
            ...
            $('#amount1').html(bit(v.amount1));

            $('#time2').html(v.time2Text)
            $('#amount2').html(bit(v.amount2));

            $('#time3').html(v.time3Text)
            $('#amount3').html(bit(v.amount3));

            $('#yue4').html(v.yue4)
            $('#amount4').html(bit(v.amount4))
            $('#jzzs4').html(v.jzzs4 + '%');

            $('#amount5').html(bit(v.amount5))
            $('#fqzs5').html(v.fqzs5 + '%')
            $('#title5').html(v.title5);

            $('#count6').html(bit(v.count6))
            $('#yqzs6').html(v.yqzs6 + '%')
            $('#title6').html(v.title6);

            $('#amount7').html(bit(v.amount7))
            ...
            var qitabaoTxt = 100 - v.day90 - v.day180 - v.day365 - v.yinhangbao - v.jijinbao;
            $('#qitabao').html(qitabaoTxt.toFixed(2) + '%')
            $('#title8').html(v.title8);

            //logical
            clearInterval(loadTimer);
            dom.loadingBox.hide();
            if(v.count6 == '0' && (v.constell == '' || v.constell == undefined)){
                dom.inviteNot.remove();
                dom.constellation.remove();
                allPages = 8;
            }else if (v.count6 == '0') {
                //邀请人数判断
                dom.inviteNot.remove();
                allPages = 9;
            }else if(v.constell == '' || v.constell == undefined){
                dom.constellation.remove();
                allPages = 9 
            }
            flipbook(allPages);
